...
**A fully functional, production-ready WhatsApp Business integration platform built with Next.js 15, Supabase, and WhatsApp Cloud API.**
...
- ✅ **Persistent Storage** - All media stored in Supabase Storage
- ✅ **Secure Access** - Time-limited signed URLs served by the backend
- ✅ **Automatic Refresh** - Expired URLs refresh automatically
- ✅ **Organized Structure** - Media organized by sender
- ✅ **Encrypted by Supabase** - Storage is encrypted at rest
...
- **Supabase Account** for database, auth, and storage ([Sign up](https://supabase.com))
...
3. Create a new bucket, for example `wachat-media-bucket`.
4. Keep the bucket private if you want server-side signed URL access only.
5. Note the bucket name for your environment configuration.
...
# ============================================
NEXT_PUBLIC_SUPABASE_URL=https://yjbepheunwaycutnnbin.supabase.co/
...
# ============================================
SUPABASE_BUCKET_NAME=wachat-media-bucket
...
2. Check `next.config.ts` has your Supabase host in `remotePatterns`
3. Confirm signed URLs are not expired
4. Test bucket permissions in Supabase Storage
5. Check Supabase service role key is correct in environment variables
