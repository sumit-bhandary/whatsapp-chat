...
import { downloadAndUploadToSupabase } from '@/lib/storage';
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const message of messages) {
      ...
      // Handle media upload to Supabase storage if it's a media message
      let storageMediaUrl = null;
      ...
      if (mediaData && mediaData.id && accessToken) {
        ...
        try {
          ...
          if (whatsappMediaUrl) {
              console.log(`Downloading and uploading ${messageType} to Supabase storage...`);
            // Validate media ID format (should be numeric)
            if (!/^\d+$/.test(mediaData.id)) {
              throw new Error(`Invalid media ID format: ${mediaData.id}`);
            }
            
            // Download from WhatsApp and upload to Supabase storage
            storageMediaUrl = await downloadAndUploadToSupabase(
              whatsappMediaUrl,
            ...
            if (storageMediaUrl) {
              console.log(`Successfully uploaded ${messageType} to Supabase storage: ${storageMediaUrl}`);
              storageUploadSuccess = true;
            } else {
              console.error(`Failed to upload ${messageType} to Supabase storage`);
            }
          } else {
      ...
      // Prepare message object for database with Supabase storage URL
      const messageObject = {
        ...
        media_data: mediaData ? JSON.stringify({
          ...mediaData,
          media_url: storageMediaUrl, // Use Supabase storage URL instead of WhatsApp URL
          uploaded_to_supabase: storageUploadSuccess, // Use the success flag instead of just checking URL presence
          upload_timestamp: storageUploadSuccess ? new Date().toISOString() : null,
          upload_error: !storageUploadSuccess && mediaData.id ? 'Failed to upload to Supabase storage' : null
        }) : null
      };

      // Store the incoming message with Supabase media URL
      ...
      if (messageError) {
        ...
        if (mediaData) {
          console.log('Media data stored with Supabase storage URL:', {
            type: mediaData.type,
            id: mediaData.id,
            uploaded_to_supabase: storageUploadSuccess,
            has_storage_url: !!storageMediaUrl,
            upload_success: storageUploadSuccess
          });
        }
      }
    }
