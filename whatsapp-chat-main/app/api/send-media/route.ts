...
import { uploadFileToSupabase, isWhatsAppSupportedFileType } from '@/lib/storage';
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const mediaIdForStorage = `upload_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        const storageUrl = await uploadFileToSupabase(file, user.id, mediaIdForStorage);
        ...
        const messageObject = {
          ...
          media_data: JSON.stringify({
            type: mediaType,
            id: mediaIdForStorage,
            mime_type: file.type,
            filename: file.name,
            caption: caption,
            media_url: storageUrl,
            uploaded_to_supabase: !!storageUrl,
