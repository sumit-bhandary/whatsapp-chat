...
export function ChatWindow({ 
  ...
  const downloadMedia = async (url: string, filename: string) => {
    try {
      // For Supabase pre-signed URLs, we can download directly
      ...
      case 'image':
        return (
          <div className={baseClasses}>
            {mediaData?.media_url && mediaData.uploaded_to_supabase ? (
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-100 dark:bg-gray-800 rounded-xl mb-2">
                ...
                {mediaData?.uploaded_to_supabase === false && (
                  <Button
                    size="sm"
                    variant="ghost"
          ...
          <div className={baseClasses}>
            <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl mb-2 min-w-[300px] max-w-[400px]">
              <Button
                ...
                disabled={!mediaData?.media_url || !mediaData.uploaded_to_supabase || isRefreshing}
              >
                {isRefreshing ? (
                  <RefreshCw className="h-5 w-5 animate-spin" />
